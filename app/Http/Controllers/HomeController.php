...
class HomeController extends Controller
{
    ...
    public function index(Request $request, $id = NULL)
    {
        ...
        $specialSalesUp = Product::whereHas('discount', function ($query) {
            $query->whereBetween('discountPercent', [10, 30])->orderBy('id', 'DESC');
        })->take(3)->get();

        $specialSalesDown = Product::whereHas('discount', function ($query) {
            $query->whereBetween('discountPercent', [40, 60])->orderBy('id', 'DESC');
        })->take(3)->get();
    ...
    public function detail(Product $product, $id = NULL, Request $request)
    {
        ...
        if ($product) {
            $id = $product->group_id;
            $this->passToView();
            $ralatedProducts = Product::where('group_id', '=', $id)->where('id','!=',$product->id)->take(8)->get();
        ...
        if ($request->isMethod('post')) {
            ...
            Mail::send('emails.contact',
                [
                    ...
                    $message->from('moloud.marzban@gmail.com', 'ContactUs');

                    $message->to('m_marzban71@yahoo.com')->subject('WebMarcket');
    ...
    {
        $this->passToView();
        $orders=Order::where('user_id','=',$user->id)->paginate('8');
