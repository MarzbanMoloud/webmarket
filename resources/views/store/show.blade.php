            ...
            <div class="product">
                <div class="product-img">
                    <div class="picture">
                        ...
                        <div class="img-overlay">
                            <a class="btn more btn-primary" href="/detail/{{$product->id}}">توضیحات
                                بیشتر</a>

                            @if ($product->quantity != 0)
                                <a class="btn buy btn-danger" href="/search/{{$product->id}}">اضافه
                                    به سبد خرید</a>
                            @endif
                        </div>
                    </div>
                ...
                <div class="main-titles no-margin">
                    @if($product->discount->discountPercent !=0)
                        <h4 class="title"><span class="striked">{{($product->price)}} تومان</span> <span
                                    class="red-clr">{{number_format(($product->price)-(($product->price)*($product->discount->discountPercent)/100))}} تومان</span></h4>
